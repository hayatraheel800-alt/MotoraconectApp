...
The phases remain numbered for architecture/history, but implementation follows product criticality rather than treating every phase as equally urgent.
...
6. Phase 11 — Payments **(implemented)**
...
We complete the current priority phase to its planned implementation scope, commit it, update its phase documentation, and then move directly to the next priority phase. Lower-priority expansion work does not interrupt the critical transaction journey.
...
Each phase should have implementation, database/schema changes where needed, documentation, and Git commits. A phase is implementation-complete before moving to the next one; full-system verification happens at the end.
